...
* **Datos Crudos (CSV):** 25 GB.
* **Datos Optimizados (Parquet):** 5 GB (Ratio de compresión aprox. 5:1).
...
* **Escenario A (Ineficiente):** Cada consulta escanea el dataset completo en CSV (25 GB por consulta).
* **Escenario B (Optimizado):** Cada consulta escanea solo las columnas necesarias en Parquet (0.5 GB por consulta).
...
* **S3 Standard storage:** `30 GB` (25GB CSV + 5GB Parquet).
...
* **Amount of data scanned per query:** `25 GB` (Athena debe leer todo el archivo porque CSV no es columnar).
...
* **Amount of data scanned per query:** `0.5 GB` (Athena usa "Partition Pruning" y lectura columnar, leyendo solo una fracción de los datos).
* **Queries per month:** `100`.
